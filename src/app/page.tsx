...
export default function PlannerPage() {
  ...
  const [ptabInsumosFecha, setPtabInsumosFecha] = useState(new Date());
  ...
  const generarExcelAguaMensual = async () => {
    ...
    diasMes.forEach((dia, idx) => {
      ...
      const row = worksheet.addRow({
        ...
        consumido: consumido || '',
        vp: vp ? parseFloat(vp.toFixed(2)) : '',
        rendimiento: rendimiento ? Number(formatAguaDisplay(rendimiento).replace(/\./g, '').replace(',', '.')) : '',
      });
      row.border = { top: { style: 'thin' }, bottom: { style: 'thin' }, left: { style: 'thin' }, right: { style: 'thin' } };
      if (idx % 2 === 0) {
        row.fill = { type: 'pattern', pattern: 'solid', fgColor: { argb: 'FFf8fafc' } };
  ...
  useEffect(() => {
   ...
   const [producidasTurnoSubTab, setProducidasTurnoSubTab] = useState('diurno');
      const [inventariosSubTab, setInventariosSubTab] = useState('diarios');
    const [inventariosMensualSubTab, setInventariosMensualSubTab] = useState('empaque');
  ...
  const [rSemanalSelectedMonth, setRSemanalSelectedMonth] = useState(rSemanalWeekStartDate.getMonth() + 1);
  const [rSemanalSelectedYear, setRSemanalSelectedYear] = useState(rSemanalWeekStartDate.getFullYear());

  useEffect(() => {
    setRSemanalSelectedMonth(rSemanalWeekStartDate.getMonth() + 1);
    setRSemanalSelectedYear(rSemanalWeekStartDate.getFullYear());
  }, [rSemanalWeekStartDate]);
  ...
  const getAguaConsumoNumber = (fechaStr: string): number => {
  ...
  const formatAguaDisplay = (value: number | string | undefined | null): string => {
                 ...
                 <div className="flex flex-col gap-2">
                   ...
                   {hasAccess(user.id, 'recipes') && (
                     ...
                     >
                      ...
                      {hasAccess(user.id, 'produccion') && (
                      <Button 
                        variant="ghost" 
                        onClick={() => { setActiveModule('produccion'); setActiveTab('produccion-view'); }}
                        className={sidebarButtonClass(activeModule === 'produccion', "bg-orange-700 hover:bg-orange-800", "shadow-orange-200/30")}
                      >
                        <div className={iconContainerClass(activeModule === 'produccion')}>
                          <Gauge className="h-4 w-4" />
                        </div>
                        <span className="uppercase text-[10px] font-black tracking-tight">Producción</span>
                      </Button>
                      )}
                      ...
                      {activeModule === 'produccion' && hasAccess(user.id, 'produccion') && (
                                ...
                                <div className="flex items-center gap-2 mb-2 no-print">
                                  <div className="flex items-center bg-slate-100/50 p-1 rounded-full h-11 border border-slate-200">
                                    {['empaque', 'materia-prima', 'insumos'].map((subTab) => (
                                      <button
                                        key={subTab}
                                        onClick={() => setInventariosMensualSubTab(subTab)}
                                        className={cn(
                                          "inline-flex items-center justify-center gap-1.5 h-9 px-2 sm:px-6 rounded-full font-bold text-[10px] uppercase tracking-widest whitespace-nowrap flex-shrink-0 outline-none focus:ring-0 border-0 select-none transition-none active:scale-95 transform-none",
                                     ...
                                     <tbody>
                                       <tr>
                                         ...
                                         <td className="px-2 py-2 text-[10px] text-slate-600 border-r border-slate-200 border-b border-slate-200">
                                           <input
                                             type="text"
                                             value={tapasData['EMP_0095-Alpla']?.totalCajas || ''}
                                         ...
                                         <td className="px-2 py-2 text-[10px] text-slate-600 border-b border-slate-200" rowSpan={2}>
                                           <input
                                             type="text"
                                             value={tapasData['EMP_0095-Alpla']?.total || ''}
                                       ...
                                       <tr>
                                         ...
                                         <td className="px-2 py-2 text-[10px] text-slate-600 border-r border-slate-200 border-t border-slate-200">
                                           <input
                                             type="text"
                                             value={tapasData['EMP_0095-Importada']?.totalCajas || ''}
                                       ...
                                       <tr>
                                         ...
                                         <td className="px-2 py-2 text-[10px] text-slate-600 border-r border-slate-200 border-t border-slate-200">
                                           <input
                                             type="text"
                                             value={tapasData['EMP_0105-Alpla']?.totalCajas || ''}
                                         ...
                                         <td className="px-2 py-2 text-[10px] text-slate-600 border-b border-slate-200" rowSpan={3}>
                                           <input
                                             type="text"
                                             value={tapasData['EMP_0105-Alpla']?.total || ''}
                                       ...
                                       <tr>
                                         ...
                                         <td className="px-2 py-2 text-[10px] text-slate-600 border-r border-slate-200 border-t border-slate-200">
                                           <input
                                             type="text"
                                             value={tapasData['EMP_0105-ImportadaEW']?.totalCajas || ''}
                                       ...
                                       <tr>
                                         ...
                                         <td className="px-2 py-2 text-[10px] text-slate-600 border-r border-slate-200 border-t border-slate-200">
                                           <input
                                             type="text"
                                             value={tapasData['EMP_0105-ImportadaTipo2']?.totalCajas || ''}
                                     ...
                                     <tbody>
                                       <tr>
                                         ...
                                         <td className="px-2 py-2 text-[10px] text-slate-600 border-r border-slate-200 border-b border-slate-200">
                                           <input
                                             type="text"
                                             value={tapasDataSemanal['EMP_0095-Alpla']?.totalCajas || ''}
                                         ...
                                         <td className="px-2 py-2 text-[10px] text-slate-600 border-b border-slate-200" rowSpan={2}>
                                           <input
                                             type="text"
                                             value={tapasDataSemanal['EMP_0095-Alpla']?.total || ''}
                                       ...
                                       <tr>
                                         ...
                                         <td className="px-2 py-2 text-[10px] text-slate-600 border-r border-slate-200 border-t border-slate-200">
                                           <input
                                             type="text"
                                             value={tapasDataSemanal['EMP_0095-Importada']?.totalCajas || ''}
                                       ...
                                       <tr>
                                         ...
                                         <td className="px-2 py-2 text-[10px] text-slate-600 border-r border-slate-200 border-t border-slate-200">
                                           <input
                                             type="text"
                                             value={tapasDataSemanal['EMP_0105-Alpla']?.totalCajas || ''}
                                         ...
                                         <td className="px-2 py-2 text-[10px] text-slate-600 border-b border-slate-200" rowSpan={3}>
                                           <input
                                             type="text"
                                             value={tapasDataSemanal['EMP_0105-Alpla']?.total || ''}
                                       ...
                                       <tr>
                                         ...
                                         <td className="px-2 py-2 text-[10px] text-slate-600 border-r border-slate-200 border-t border-slate-200">
                                           <input
                                             type="text"
                                             value={tapasDataSemanal['EMP_0105-ImportadaEW']?.totalCajas || ''}
                                       ...
                                       <tr>
                                         ...
                                         <td className="px-2 py-2 text-[10px] text-slate-600 border-r border-slate-200 border-t border-slate-200">
                                           <input
                                             type="text"
                                             value={tapasDataSemanal['EMP_0105-ImportadaTipo2']?.totalCajas || ''}
                                            ...
                                            <tbody>
                                              <tr>
                                                ...
                                                <td className="px-2 py-2 text-[10px] text-slate-600 border-r border-slate-200 border-b border-slate-200">
                                                  <input
                                                    type="text"
                                                    value={tapasDataMensualEmpaque['EMP_0095-Alpla']?.totalCajas || ''}
                                                ...
                                                <td className="px-2 py-2 text-[10px] text-slate-600 border-b border-slate-200" rowSpan={2}>
                                                  <input
                                                    type="text"
                                                    value={tapasDataMensualEmpaque['EMP_0095-Alpla']?.total || ''}
                                              ...
                                              <tr>
                                                ...
                                                <td className="px-2 py-2 text-[10px] text-slate-600 border-r border-slate-200 border-t border-slate-200">
                                                  <input
                                                    type="text"
                                                    value={tapasDataMensualEmpaque['EMP_0095-Importada']?.totalCajas || ''}
                                              ...
                                              <tr>
                                                ...
                                                <td className="px-2 py-2 text-[10px] text-slate-600 border-r border-slate-200 border-t border-slate-200">
                                                  <input
                                                    type="text"
                                                    value={tapasDataMensualEmpaque['EMP_0105-Alpla']?.totalCajas || ''}
                                                ...
                                                <td className="px-2 py-2 text-[10px] text-slate-600 border-b border-slate-200" rowSpan={3}>
                                                  <input
                                                    type="text"
                                                    value={tapasDataMensualEmpaque['EMP_0105-Alpla']?.total || ''}
                                              ...
                                              <tr>
                                                ...
                                                <td className="px-2 py-2 text-[10px] text-slate-600 border-r border-slate-200 border-t border-slate-200">
                                                  <input
                                                    type="text"
                                                    value={tapasDataMensualEmpaque['EMP_0105-ImportadaEW']?.totalCajas || ''}
                                              ...
                                              <tr>
                                                ...
                                                <td className="px-2 py-2 text-[10px] text-slate-600 border-r border-slate-200 border-t border-slate-200">
                                                  <input
                                                    type="text"
                                                    value={tapasDataMensualEmpaque['EMP_0105-ImportadaTipo2']?.totalCajas || ''}
                       ...
                       {activeModule === 'procesos' && (isDemon || user.id === 'jaime.r' || user.id === 'proc1.mds' || user.id === 'procj.mds' || user.id === 'procs2.mds' || user.id === 'maria.mds' || user.id === 'alex.mds' || user.id === 'proc.mds' || user.id === 'procs1.mds' || user.id === 'proc2.mds' || user.id === 'prodtg.mds') && (
                        <div className="flex flex-col h-full">
                            ...
                            <div className="flex items-center gap-2 no-print mb-2">
                              <div className="flex items-center bg-slate-100/50 p-1 rounded-full h-11 border border-slate-200">
                                   ...
                                   >
                                     ...
                                     {tab === 'r-semanal' && <CalendarRange className="h-3.5 w-3.5" />}
                                     {tab === 'r-mensual' && <BarChart3 className="h-3.5 w-3.5" />}
                                     <span className="hidden sm:inline">{tab === 'agua' ? 'Agua' : tab === 'insumos' ? 'Insumos' : tab === 'r-semanal' ? 'R Semanal' : 'R mensual'}</span>
                                   </button>
                                 ))}
                              </div>
                            </div>
                                  ...
                                  <div className="flex items-center justify-end no-print gap-2">
                                    <Popover>
                                      <PopoverTrigger asChild>
                                        <button className="inline-flex items-center gap-2 h-9 pl-3 pr-4 rounded-full font-bold text-[10px] whitespace-nowrap flex-shrink-0 outline-none select-none border-0 bg-white text-slate-700 shadow-sm transition-none">
                                          <CalendarIcon className="h-3.5 w-3.5 text-primary" />
                                          {format(ptabInsumosFecha || new Date(), 'd/M/yyyy')}
                                        </button>
                                      </PopoverTrigger>
                                      <PopoverContent className="p-0 w-auto" align="end">
                                        <Calendar mode="single" selected={ptabInsumosFecha} onSelect={setPtabInsumosFecha} required locale={es} />
                                      </PopoverContent>
                                    </Popover>
                                  </div>
                                  <div className="flex-1 rounded-2xl border border-slate-100 bg-white overflow-x-auto">
                                          ...
                                          {insumosQuimicos.map((item) => (
                                            <tr key={item.nombre} className="border-b border-slate-100 hover:bg-slate-50/50">
                                              ...
                                              <td className="px-2 py-2 font-bold text-slate-700 border border-slate-100 whitespace-nowrap">{item.nombre}</td>
                                               {Array.from({ length: 3 }).map((_, idx) => (
                                                 <td key={idx} className="px-1 py-1 text-center border border-slate-300 bg-white">
                                                   <input
                                                     type="text"
                                                     value={ptabInsumosStore.data[`${ptabInsumosFecha ? format(ptabInsumosFecha, 'yyyy-MM-dd') : ''}-${item.nombre}-${idx}`] || ''}
                                                     onChange={(e) => handlePtabInsumosChange(ptabInsumosFecha, item.nombre, idx, e.target.value)}
                                               ...
                                               <td className="px-1 py-1 text-center border border-slate-300 bg-white font-black text-slate-700">
                                                 {(() => {
                                                   ...
                                                   const invInicial = Number(String(ptabInsumosStore.data[`${dateStr}-${item.nombre}-0`] || '0').replace(/\./g, '').replace(',', '.')) || 0;
                                                   const recepcion = Number(String(ptabInsumosStore.data[`${dateStr}-${item.nombre}-1`] || '0').replace(/\./g, '').replace(',', '.')) || 0;
                                                   const invFinal = Number(String(ptabInsumosStore.data[`${dateStr}-${item.nombre}-2`] || '0').replace(/\./g, '').replace(',', '.')) || 0;
                                               ...
                                               {Array.from({ length: 3 }).map((_, idx) => (
                                                 <td key={`noct-${idx}`} className={`px-1 py-1 text-center border border-slate-300 bg-white ${idx === 0 ? 'border-l-4 border-black' : ''}`}>
                                                   <input
                                                     type="text"
                                                     value={ptabInsumosStore.data[`${ptabInsumosFecha ? format(ptabInsumosFecha, 'yyyy-MM-dd') : ''}-${item.nombre}-${idx + 4}`] || ''}
                                                     onChange={(e) => handlePtabInsumosChange(ptabInsumosFecha, item.nombre, idx + 4, e.target.value)}
                                                     className="w-full min-w-[14ch] h-9 text-center text-[11px] font-bold text-slate-900 bg-white border border-slate-300 rounded focus:outline-none focus:border-primary"
                                                   />
                                                 </td>
                                               ))}
                                               <td className="px-1 py-1 text-center border border-slate-300 bg-white font-black text-slate-700">
                                                 {(() => {
                                                   const dateStr = ptabInsumosFecha ? format(ptabInsumosFecha, 'yyyy-MM-dd') : '';
                                                   const invInicial = Number(String(ptabInsumosStore.data[`${dateStr}-${item.nombre}-4`] || '0').replace(/\./g, '').replace(',', '.')) || 0;
                                                   const recepcion = Number(String(ptabInsumosStore.data[`${dateStr}-${item.nombre}-5`] || '0').replace(/\./g, '').replace(',', '.')) || 0;
                                                   const invFinal = Number(String(ptabInsumosStore.data[`${dateStr}-${item.nombre}-6`] || '0').replace(/\./g, '').replace(',', '.')) || 0;
                                                   const consumo = (invInicial + recepcion) - invFinal;
                                                   return consumo ? consumo.toLocaleString('es-VE', { minimumFractionDigits: 2, maximumFractionDigits: 2 }) : '';
                                                 })()}
                                               </td>
                                              <td className="px-2 py-2 text-center border border-slate-100 font-black text-slate-700">
                                                 {(() => {
                                                   const dateStr = ptabInsumosFecha ? format(ptabInsumosFecha, 'yyyy-MM-dd') : '';
                                                   const invInicialDiurno = Number(String(ptabInsumosStore.data[`${dateStr}-${item.nombre}-0`] || '0').replace(/\./g, '').replace(',', '.')) || 0;
                                                   const recepcionDiurno = Number(String(ptabInsumosStore.data[`${dateStr}-${item.nombre}-1`] || '0').replace(/\./g, '').replace(',', '.')) || 0;
                                                   const invFinalDiurno = Number(String(ptabInsumosStore.data[`${dateStr}-${item.nombre}-2`] || '0').replace(/\./g, '').replace(',', '.')) || 0;
                                                   const invInicialNocturno = Number(String(ptabInsumosStore.data[`${dateStr}-${item.nombre}-4`] || '0').replace(/\./g, '').replace(',', '.')) || 0;
                                                   const recepcionNocturno = Number(String(ptabInsumosStore.data[`${dateStr}-${item.nombre}-5`] || '0').replace(/\./g, '').replace(',', '.')) || 0;
                                                   const invFinalNocturno = Number(String(ptabInsumosStore.data[`${dateStr}-${item.nombre}-6`] || '0').replace(/\./g, '').replace(',', '.')) || 0;
                          ...
                          {procesosSubTab === 'ptab' && ptabTab !== 'agua' && ptabTab !== 'insumos' && ptabTab !== 'r-semanal' && ptabTab !== 'r-mensual' && (
                            <div className="flex-1 bg-white rounded-[2.5rem] p-4">
                              <div className="flex-1 rounded-2xl bg-slate-50/50 border border-slate-100">
                                <div className="flex flex-col h-full gap-3">
                                         ...
                                         <div className="flex-1 bg-white rounded-[2.5rem] p-4">
                                           <div className="flex-1 rounded-2xl bg-slate-50/50 border border-slate-100">
                                             <div className="flex flex-col h-full gap-3">
                                                <div className="flex items-center justify-end no-print gap-2">
                                                  <Popover>
                                                    <PopoverTrigger asChild>
                                                      <button className="inline-flex items-center gap-2 h-9 pl-3 pr-4 rounded-full font-bold text-[10px] whitespace-nowrap flex-shrink-0 outline-none select-none border-0 bg-white text-slate-700 shadow-sm transition-none">
                                                        <CalendarIcon className="h-3.5 w-3.5 text-primary" />
                                                        {rSemanalMonthOptions.find(m => m.value === String(rSemanalSelectedMonth))?.label} {rSemanalSelectedYear}
                                                      </button>
                                                    </PopoverTrigger>
                                                ...
                                                <div className="flex-1 rounded-2xl border border-slate-100 bg-white overflow-x-auto">
                                                  <div className="mb-2">
                                                    <span className="text-slate-700 font-black text-sm uppercase tracking-widest">Semana {getISOWeek(rSemanalWeekStartDate)} · {rSemanalMonthOptions.find(m => m.value === String(rSemanalSelectedMonth))?.label} {rSemanalSelectedYear}</span>
                                                  </div>
                                                  <table className="w-full border-collapse text-[11px]">
                                                    <thead>
...
function calcularTotalesDiario(informesOperacionales: any[], tasks: any[], realProduction: any, lineSpeeds: any, fecha?: Date, planificadasPorDia?: Record<string, Record<string, Record<number, { diurno: number, nocturno: number }>>>, ordenes?: any[], semanaFechas?: string[], ordenesSap?: any[], turno: 'DIURNO' | 'NOCTURNO' | 'DIARIO' = 'DIARIO') {
  ...
  lineas.forEach((lineaNombre, idx) => {
    ...
    const alcanceTD = (ordenesSap || []).reduce((acc: number, orden: any) => {
      ...
      if (isNaN(ordenLinea) || ordenLinea !== lineaNum) return acc;
    ...
    const alcanceTN = (ordenesSap || []).reduce((acc: number, orden: any) => {
      const ordenLinea = Number(orden.linea);
      if (isNaN(ordenLinea) || ordenLinea !== lineaNum) return acc;
      return acc + (orden.dias || [])
        .filter((dia: any) => fechaSet.has(dia.fechaInicio))
...
function getResumenPorLinea(informesOperacionales: any[], tasks: any[], realProduction: any, lineSpeeds: any, fecha?: Date, planificadasPorDia?: Record<string, Record<string, Record<number, { diurno: number, nocturno: number }>>>, ordenes?: any[], semanaFechas?: string[], ordenesSap?: any[]) {
  let informeDelDia: any[] = [];
  let diaPlanificada: any = {};
  if (semanaFechas && semanaFechas.length > 0) {
    informeDelDia = (informesOperacionales || []).filter((r: any) => semanaFechas.includes(String(r.fecha || '')));
...
function TablaResumenPorLinea({ informesOperacionales, tasks, realProduction, lineSpeeds, fecha, planificadasPorDia, ordenes, semanaFechas, ordenesSap }: any) {
  const datos = getResumenPorLinea(informesOperacionales, tasks, realProduction, lineSpeeds, fecha, planificadasPorDia, ordenes, semanaFechas, ordenesSap);
...
function ReporteTurnoTabla({ informesOperacionales, tasks, realProduction, lineSpeeds, turno = 'DIURNO', fecha, planificadasPorDia, ordenes, velocidadesDt, hrsPagadasDia, hrsProgramadasDia, allTasks, ordenesSap }: any) {
   const data = useReportData(informesOperacionales, tasks, realProduction, lineSpeeds, turno, fecha, planificadasPorDia, ordenes, velocidadesDt, hrsPagadasDia, hrsProgramadasDia, undefined, allTasks, ordenesSap);
