...
export function MonthlyReport({ realProduction, selectedMonth, selectedYear }: MonthlyReportProps) {
  ...
  return (
    <div className="bg-white p-8 w-full min-h-screen">
      <div className="mb-8 border-b-2 border-slate-900 pb-6 flex justify-between items-center">
        <div className="flex-1">
          <h1 className="text-3xl font-headline font-black text-slate-900 leading-tight">RESUMEN MENSUAL DE PRODUCCIÓN</h1>
          <p className="text-primary font-black text-lg uppercase tracking-widest">Reporte Ejecutivo de Cajas Reales</p>
        </div>
        <div className="flex-1 flex justify-center">
          {glupLogo && <Image src={glupLogo.imageUrl} alt="Logo" width={180} height={70} className="object-contain" />}
        </div>
        <div className="flex-1 text-right">
          <p className="text-[10px] font-black text-primary uppercase tracking-[0.2em] mb-1">Confidencial - Gerencia de Planta</p>
          <p className="text-xl font-black text-slate-900">{monthName}</p>
        </div>
      </div>
        ...
        <table className="w-full border-collapse text-[10pt]">
          <thead>
            <tr className="bg-[#4a7ebb] text-white font-black uppercase">
              <th className="px-4 py-3 border border-slate-900 text-left min-w-[200px]">SABOR / PRODUCTO</th>
              {ALL_LINES_SUMMARY.slice(0, 4).map(l => (
                <th key={l} className="px-2 py-3 border border-slate-900 text-center">LÍNEA {l}</th>
              ))}
              <th className="px-4 py-3 border border-slate-900 text-center bg-[#2f5597]">TOTAL 2L</th>
            ...
            {PRODUCT_LIST.map((flavor, idx) => {
              ...
              return (
                <tr key={idx} className="font-bold text-slate-800">
                  <td className="px-4 py-2 border border-slate-300 uppercase bg-slate-50">{flavor}</td>
                  {lineVals.slice(0, 4).map((val, lIdx) => (
                    <td key={lIdx} className="px-2 py-2 border border-slate-300 text-center tabular-nums">
                      {val > 0 ? val.toLocaleString('es-ES') : '0'}
                    </td>
                  ))}
                  <td className="px-4 py-2 border border-slate-300 text-center tabular-nums bg-[#dce6f1] font-black">
                    {total2L > 0 ? total2L.toLocaleString('es-ES') : '0'}
                  </td>
                  {lineVals.slice(4).map((val, lIdx) => (
                    <td key={lIdx + 4} className="px-2 py-2 border border-slate-300 text-center tabular-nums">
                      {val > 0 ? val.toLocaleString('es-ES') : '0'}
                    </td>
                  ))}
                  <td className="px-4 py-2 border border-slate-300 text-center tabular-nums bg-[#dce6f1] font-black">
          ...
          <tfoot className="bg-[#dce6f1] text-slate-900 font-black">
            <tr>
              <td className="px-4 py-4 border border-slate-900 uppercase">TOTAL POR LINEA</td>
              {ALL_LINES_SUMMARY.slice(0, 4).map(l => {
                const colTotal = PRODUCT_LIST.reduce((acc, flavor) => acc + (monthlyData[flavor]?.[l] || 0), 0);
                return (
                  <td key={l} className="px-2 py-4 border border-slate-900 text-center tabular-nums">
                    {colTotal.toLocaleString('es-ES')}
                  </td>
                );
              })}
              <td className="px-4 py-4 border border-slate-900 text-center tabular-nums bg-[#b8cce4]">
      ...
      <div className="mt-12 flex justify-between items-end border-t border-slate-200 pt-4 text-[8px] font-black text-slate-400 uppercase tracking-widest">
        <div className="space-y-1">
          <p>SISTEMA DE GESTIÓN DE PLANTA - PLAN SEMANAL PRO</p>
          <p>EMITIDO: {format(new Date(), 'dd/MM/yyyy HH:mm:ss')}</p>
        </div>
