...
export function JarabesModule({ onPrintStandard, onPrintPromedio, onPrintWeeklyStandard, onPrintWeeklyPromedio, weekStartDate }: { onPrintStandard?: (html: string) => void; onPrintPromedio?: (html: string) => void; onPrintWeeklyStandard?: (html: string) => void; onPrintWeeklyPromedio?: (html: string) => void; weekStartDate?: Date }) {
  ...
  const [promKgFactors, setPromKgFactors] = useState<Record<string, number>>(() => {
    try {
      const saved = localStorage.getItem('jarabes-promKgFactors');
  ...
  const buildWeeklyStandardHtml = (chartImage?: string | null): string => {
    ...
    const buildLetterPdf = (canvas: HTMLCanvasElement, marginMM = 10) => {
      ...
      const fitW = contentW / (imgPxW / 2);
      const fitH = contentH / (imgPxH / 2);
    ...
    const handleExportWeeklyPDFStandard = async () => {
      try {
        ...
        const semanaTitleEst = cardEl.querySelector('#semana-title-estandar') as HTMLElement | null;
        if (pdfButtonEst) pdfButtonEst.style.visibility = 'hidden';
        if (semanaTitleEst) semanaTitleEst.style.visibility = 'hidden';
        ...
        if (pdfButtonEst) pdfButtonEst.style.visibility = 'visible';
        if (semanaTitleEst) semanaTitleEst.style.visibility = 'visible';
            ...
            <table>
               ...
               const handleExportWeeklyPDFPromedio = async () => {
                try {
                  ...
                  const semanaTitleProm = cardEl.querySelector('#semana-title-promedio') as HTMLElement | null;
                  if (pdfButtonProm) pdfButtonProm.style.visibility = 'hidden';
                  if (semanaTitleProm) semanaTitleProm.style.visibility = 'hidden';
                  ...
                  if (semanaTitleProm) semanaTitleProm.style.visibility = 'visible';
  ...
  const standardChartData = useMemo(() => {
    if (!weekDays.length) return [];
    const kgPromFactor = 50;
    return weekDays.map((day) => {
      const dateStr = format(day, 'yyyy-MM-dd');
      const dUbb = loadDayDataWithCarryOver(dateStr, 'ubb');
      const dSugar = loadDayDataWithCarryOver(dateStr, 'sugar');
      const dTanks = loadDayDataWithCarryOver(dateStr, 'tanks');
      const m = computePlannerMetrics(dUbb, dSugar, dTanks, '', kgPromFactor);
                                 ...
                                 <div className="flex flex-col flex-1 min-h-[529px] bg-slate-50/40 rounded-2xl border-2 border-slate-200 p-4 relative" ref={standardChartRef}>
                                   <ChartContainer config={chartConfig} className="flex-1 w-full">
                                     <ResponsiveContainer width="100%" height="100%">
                                         <ComposedChart data={standardChartData} margin={{ top: 40, right: 30, left: 0, bottom: 20 }}>
                                         ...
                                         <Tooltip content={<ChartTooltipContent />} cursor={{ fill: '#f8fafc' }} />
                                          ...
                                          <Bar yAxisId="left" dataKey="fisico" fill="#dbeafe" radius={[4, 4, 0, 0]} barSize={30} name="Físico" />
                                   ...
                                   <div className="flex justify-center gap-10 py-3 border-t-2 border-slate-200 bg-white shadow-md rounded-full mx-6 mb-4 shrink-0">
                                       ...
                                       <div className="flex items-center gap-2">
                                         <div className="w-5 h-5 bg-[#dbeafe] rounded shadow-sm border border-[#dbeafe]/30" />
