...
import { Bar, CartesianGrid, XAxis, YAxis, Tooltip, ResponsiveContainer, Line, ComposedChart, Legend } from 'recharts';
...
export default function PlannerPage() {
  ...
  const rSemanalChartData = useMemo(() => {
    ...
    return days.map((day) => {
      ...
      return {
        name: format(day, 'EEE d/M', { locale: es }),
        ConsumoFisico: Number(ptabAguaStore.data[`${dateStr}-total`]) || 0,
        ConsumoTeorico: vp || 0,
        Rendimiento: consumido > 0 ? Number((vp / consumido).toFixed(4)) : 0,
      };
    });
                              ...
                              <div className="flex flex-col h-full">
                                ...
                                <div className="mt-4 bg-white rounded-2xl border border-slate-100 p-4">
                                  <ResponsiveContainer width="100%" height={220}>
                                    <ComposedChart data={rSemanalChartData}>
                                      ...
                                      <Tooltip />
                                      <Legend />
                                      <Bar yAxisId="left" dataKey="ConsumoFisico" name="Consumo Fisico" fill="#0f172a" radius={[2, 2, 0, 0]} />
                                      <Bar yAxisId="left" dataKey="ConsumoTeorico" name="Consumo Teorico" fill="#64748b" radius={[2, 2, 0, 0]} />
                                      <Line yAxisId="right" type="monotone" dataKey="Rendimiento" name="Rendimiento" stroke="#16a34a" strokeWidth={2} />
                                    </ComposedChart>
                                  </ResponsiveContainer>
                                </div>
                              </div>
